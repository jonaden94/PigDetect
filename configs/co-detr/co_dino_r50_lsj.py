...
# # to get pretrained resnet weights, which might be used when training co_dino from scratch: wget https://download.pytorch.org/models/resnet50-0676ba61.pth
# resnet50_path = 'data/pretrained_weights/codino_coco/resnet50-0676ba61.pth'
# # to start training from coco pretrained checkpoint: wget https://download.openmmlab.com/mmdetection/v3.0/codetr/co_dino_5scale_r50_lsj_8xb2_1x_coco/co_dino_5scale_r50_lsj_8xb2_1x_coco-69a72d67.pth
# load_from = 'data/pretrained_weights/codino_coco/co_dino_5scale_r50_lsj_8xb2_1x_coco-69a72d67.pth'
...
# custom settings
data_root = 'data/PigDetect/'
data_prefix = 'images/'
ann_file_train = 'train.json'
ann_file_val = 'val.json'
ann_file_test = 'test.json'
image_size = (960, 960)
with_mask = False
pad_mask = False
max_epochs = 100
...
num_workers = 2
interval_epochs = 1
logging_iter = int(2481 / 16) # assuming batch size of 2 and using 4 gpus, this logs twice per epoch
...
default_hooks = dict(
    logger=dict(interval=logging_iter, type='LoggerHook'),
    checkpoint=dict(
        by_epoch=True,
...
batch_augments = [
    dict(type='BatchFixedSizePad', size=image_size, pad_mask=pad_mask)
]
model = dict(
    type='CoDETR',
    ...
    backbone=dict(
        ...
        norm_eval=True,
        style='pytorch',
        # init_cfg=dict(type='Pretrained', checkpoint=resnet50_path)), # if training from scratch, you might use this pretrained resnet checkpoint
        init_cfg=None),
    neck=dict(
        type='ChannelMapper',
...
param_scheduler = [
    dict(
        ...
        by_epoch=True,
        milestones=[max_epochs - last_epochs],
        gamma=0.1)
]
